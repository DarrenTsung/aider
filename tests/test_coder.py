...
class TestCoder(unittest.TestCase):
    ...
    def test_check_for_filename_with_line_number_mentions(self):
        with GitTemporaryDirectory():
            ...
            # Check that mentions for the same file don't add the full file when find_mentions_for_partial_files_in_chat is False
            coder.check_for_file_mentions("Also check file1.txt:10 and also file1.txt please", find_mentions_for_partial_files_in_chat=False)
            self.assertEqual(coder.abs_fnames, set([str(Path(coder.root) / fname1)]))
            self.assertIn("file1.txt:15-25", coder.get_files_content().splitlines())

            # Check handling merging ranges, multiple mentions, column numbers, and out-of-bounds.
            coder.check_for_file_mentions("Also check file1.txt:25:2 and file1.txt:50", find_mentions_for_partial_files_in_chat=True)
            self.assertEqual(coder.abs_fnames, set([str(Path(coder.root) / fname1)]))
